...
public class Sorts {
    ...
    public static <T extends Comparable<? super T>> void heapify(T[] arr, int max, int len) {
        int l_child = 2 * max + 1;
        int r_child = 2 * max +2;

        if ((l_child >= len) && (r_child >= len)) {
            return;
        }
        if ((l_child > len) && (arr[l_child].compareTo(arr[max]) > 0)) {
            swap(arr, max, l_child);
        } else if ((r_child > len) && (arr[r_child].compareTo(arr[max]) > 0)) {
            swap(arr, max, r_child);
        }

        heapify(arr, l_child, len);
        heapify(arr, r_child, len);
    }

    public static <T extends Comparable<? super T>> void heapSortHelper(T[] arr) {
